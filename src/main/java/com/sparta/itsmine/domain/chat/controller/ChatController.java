...
public class ChatController {
    ...
    // 백프레셔를 위한 메시지 큐
    private final BlockingQueue<MessageRequestDto> messageQueue = new LinkedBlockingQueue<>(
            100); // 큐 크기를 100으로 제한
    ...
    protected void processMessages() {
        ...
        while (!messageQueue.isEmpty()) {
            try {
                MessageRequestDto requestDto = messageQueue.poll(2,
                        TimeUnit.SECONDS); // 큐에서 메시지를 가져옴
                ...
                if (requestDto != null) {
                    ...
                    if (processedMessages.putIfAbsent(requestDto.getMessageId(), true) != null) {
                        log.warn("Duplicate message detected during processing, ignoring: {}",
                    ...
                    messagingTemplate.convertAndSend(
                            "/topic/chat.message/" + requestDto.getRoomId(), requestDto);
    ...
    public void receiveMessage(MessageRequestDto requestDto) {
        ...
        // 중복 메시지 여부 확인
        if (processedMessages.putIfAbsent(requestDto.getMessageId(), true) != null) {
            log.warn("Duplicate message detected during receive, ignoring: {}",
                    requestDto.getMessageId());
            return;
        }
